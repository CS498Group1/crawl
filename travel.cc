...
void do_interlevel_travel()
{
    ...
    if (!can_travel_interlevel())
    {
        ...
        else if ((!you.running.pos.x || !you.running.pos.y) && level_id::current() != BRANCH_SEWER && 
            level_id::current() != BRANCH_OSSUARY && level_id::current() != BRANCH_BAILEY && level_id::current() != BRANCH_GAUNTLET
            && level_id::current() != BRANCH_ICE_CAVE && level_id::current() != BRANCH_VOLCANO && level::current() != BRANCH_DESOLATION
            && level_id::current() != BRANCH_BAZAAR && level_id::current() != BRANCH_TROVE && level_id::current() != BRANCH_ZIGGURAT)
        {
            mpr("Sorry, you can't auto-travel out of here.");
            return;
        }
